...
file_path <- "F:/behavior_result/graph_data/LR/LR_st_vio.mat"
data_mat <- readMat(file_path)

alpha_vio <- data_mat$alpha.vio
alpha_stable <- data_mat$alpha.stable
...
p <- ggplot(data, aes(x=Condition, y=Value, fill=Condition)) +
  ...
  scale_y_continuous(breaks = seq(0.03, 0.036, by = 0.001), limits = c(0.03-0.001/7, 0.036+0.001/7), expand = c(0, 0)) +  
  geom_signif(comparisons = list(c("Volatile", "Stable")),
              map_signif_level = TRUE, textsize = 8, vjust = -0.2)  

print(p)
ggsave(filename = "C:/Users/wcy13/graph/Volatile_vs_Stable.png", 
       plot = p, 
       dpi = 1500,   
       width = 5,   
       height = 6,  
